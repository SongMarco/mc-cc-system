...
예시: `feat: 약사 리스트 조회 시 페이징 쿼리 파라미터를 추가하다`

커밋 본문(description)에는 변경 사항을 bullet point로 요약한다.
예시:
```
refactor: push-n-pr skill을 slash command로 마이그레이션하고 commit command를 추가하다

- push-n-pr skill을 /push, /pr, /push-n-pr 슬래시 커맨드 3개로 분리
- /commit 슬래시 커맨드 신규 추가 (컨벤션 기반 커밋 메시지 생성)
- .gitignore에 .claude/commands/ 디렉토리 추적 예외 추가
```
